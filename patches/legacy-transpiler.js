...
var LegacyTranspiler = (() => {
  ...
  function getOptions(opts) {
    ...
    if (options2.ecmaVersion === "latest") {
      ...
      if (!warnedAboutEcmaVersion && typeof console === "object" && console.warn) {
        ...
        console.warn("Defaulting to 2020");
      }
  ...
  var GENERATOR = {
    ...
    ForStatement(node, state) {
      state.write("for (");
      if (node.init != null) {
        const { init: init2 } = node;
        if (init2.type[0] === "V") {
          formatVariableDeclaration(state, init2);
        } else {
          this[init2.type](init2, state);
        }
      }
      state.write("; ");
      if (node.test) {
  ...
  // src/utils/patchFetch.ts
  function patchFetch() {
    var _nativeFetch = window.fetch.bind(window);
    function _cleanFetch(url, opts) {
  ...
  function transformStaticImports(ast) {
  ...
  var createDynamicImportsVisitor = () => {
    return {
  ...
  // src/transforms/removeExport.ts
  function transformExports(ast, options2) {
    const exports = [];
    ast.body = ast.body.flatMap((node) => {
    ...
    if (exports.length > 0) {
      const props = exports.map((e) => {
        const key = { type: "Identifier", name: e.key, start: 0, end: 0 };
        const value = { type: "Identifier", name: e.value, start: 0, end: 0 };
        return {
          type: "Property",
          key,
          value,
  ...
  // src/transforms/wrapAsyncIIFE.ts
  function transformWrapAsyncIIFE(ast) {
    const asyncFunc = {
      type: "FunctionExpression",
      id: null,
      params: [],
  ...
  // src/transforms/mergeVisitors.ts
  ...
  function exportModule(source, exports) {
    console.log("[exportModule] ".concat(source, " \u2192"), exports);
